...
execute if score #enchantment_lvl ifl.dummy matches 1 as @e[type=minecraft:armor_stand,tag=tcc.temp_item_holder,sort=nearest,limit=1] if predicate tcc:entity/wearing/single_lvl_enchantment run data modify storage ifl:storage lore_add set value '{"nbt":"ench_name","storage":"ifl:storage","interpret":true}'
